...
function checkForNewMessages() {
    // In a real P2P app, this would use WebRTC to retrieve messages
    // For this simulation, we're just checking localStorage
    
    app.contacts.forEach(contact => {
        const key = `${LS.MSG_PREFIX}${contact.id}`;
        ...
        if (savedMessages) {
            ...
            const currentCount = (app.messages[contact.id] || []).length;
            const newCount = messages.length;
            
            if (newCount > currentCount) {
                // New messages!
                app.messages[contact.id] = messages;
...
function showChatHistory(contactId) {
    ...
    const history = app.messages[contactId] || [];
...
function saveMessage(msg) {
    ...
    if (!app.messages[contactId]) {
        app.messages[contactId] = [];
    }
    
    app.messages[contactId].push(msg);
    
    // Save to localStorage
    localStorage.setItem(
        `${LS.MSG_PREFIX}${contactId}`, 
        JSON.stringify(app.messages[contactId])
    );
}
...
function loadAllMessages() {
    ...
    app.contacts.forEach(contact => {
        ...
        app.messages[contact.id] = saved ? JSON.parse(saved) : [];
    });
}
