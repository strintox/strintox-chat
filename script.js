...
function initFirebase() {
    ...
    firebaseConfig = {
        apiKey: "AIzaSyC2yRJEXnZpSrroSn8RJOmXP_ZlVbEYi2o",
        authDomain: "weather-chat-app-4d1e5.firebaseapp.com",
        databaseURL: "https://weather-chat-app-4d1e5-default-rtdb.europe-west1.firebasedatabase.app",
        projectId: "weather-chat-app-4d1e5",
        storageBucket: "weather-chat-app-4d1e5.appspot.com",
        messagingSenderId: "173183152982",
        appId: "1:173183152982:web:2b4f5ece2c58d2b78d5bca"
    };
...
function loadFirebaseSDK(callback) {
    ...
    firebaseApp.onload = function() {
        // После загрузки основной библиотеки, загружаем модуль базы данных
        const firebaseDB = document.createElement('script');
        firebaseDB.src = 'https://www.gstatic.com/firebasejs/8.10.1/firebase-database.js';
        firebaseDB.onload = callback;
...
function saveUsername() {
    ...
    if (database) {
        ...
        // Если Firebase еще не инициализирован
        username = name;
        localStorage.setItem('chat_username', username);
        ...
        // Уведомление об ошибке подключения к чату
        showToast('Подключение к чату не установлено. Перезагрузите страницу.', 'error');
    }
}
...
function listenForMessages() {
    ...
    // Сначала получаем последние сообщения
    messagesRef.orderByChild('timestamp').limitToLast(MESSAGES_LIMIT).once('value', snapshot => {
        // Очищаем чат
        chatMessages.innerHTML = '';
        
        // Добавляем приветственное сообщение, если нет других сообщений
        if (!snapshot.exists()) {
    ...
    messagesRef.orderByChild('timestamp').startAt(Date.now()).on('child_added', snapshot => {
        ...
        // Прокручиваем к последнему сообщению, если чат активен
        if (chatIsActive) {
            scrollToBottom();
        } else {
            // Уведомляем пользователя о новом сообщении
            showToast(`Новое сообщение от ${message.author || 'чата'}`, 'info');
        }
    });
}
