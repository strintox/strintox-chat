...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Установка слушателя для переключения темы
...
function updateMarkerWeather(cityName, marker) {
    ...
    fetch(`https://api.openweathermap.org/data/2.5/weather?q=${apiCityName}&appid=${API_KEY}&units=metric`)
        .then(response => {
            if (!response.ok) {
...
function getCurrentWeather(city) {
    ...
    fetch(`https://api.openweathermap.org/data/2.5/weather?q=${apiCityName}&appid=${API_KEY}&units=metric`)
        ...
        .then(data => {
            ...
            feelsLikeElement.textContent = `Ощущается как: ${Math.round(data.main.feels_like)} °C`;
...
function getForecast(city) {
    ...
    fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${apiCityName}&appid=${API_KEY}&units=metric`)
        .then(response => {
            if (!response.ok) {
    ...
    Object.keys(dailyForecasts).forEach(day => {
        ...
        forecastItem.innerHTML = `
            ...
            <div class="forecast-details">
                <div><i class="fas fa-wind"></i> ${forecast.wind.speed.toFixed(1)} м/с</div>
