 ...
 * Чат реализован с использованием Supabase для работы на GitHub Pages
 */
...
const MESSAGES_LIMIT = 50; // Количество сообщений для отображения
let chatIsActive = false;
let supabase = null;
let messagesSyncInterval = null;
...
function initSupabase() {
    ...
    loadSupabaseSDK(() => {
        // Параметры подключения к Supabase (публичные ключи)
        const SUPABASE_URL = 'https://mjbpxzwkvkccfgtsrqty.supabase.co';
        const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1qYnB4endrdmtjY2ZndHNycXR5Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3MDAzMDI0MDQsImV4cCI6MjAxNTg3ODQwNH0.mBejkiKqQyYtjXCCv5s3mXobw9tcKSjwmMJbgBFRJVA';
        ...
        supabase.auth.signUp({
            email: `anonymous-${Date.now()}@weatherchat.com`,
            password: `anon-${Date.now()}-${Math.random().toString(36).substring(2, 15)}`
        })
        .then(() => {
            console.log('Анонимный пользователь создан');
            
            // Проверка существования таблицы и создание если нужно
            ensureMessagesTable()
                .then(() => {
                    // Начинаем получать сообщения
                    loadMessages();
                    
                    // Настраиваем регулярную синхронизацию сообщений
                    messagesSyncInterval = setInterval(loadMessages, 5000);
                })
                .catch(error => {
                    console.error('Ошибка при проверке таблицы:', error);
                    addSystemMessageToUI('Ошибка подключения к чату. Попробуйте перезагрузить страницу.');
                });
        })
        .catch(error => {
...
// Динамическая загрузка Supabase SDK
...
// Проверка существования таблицы сообщений и создание если нужно
async function ensureMessagesTable() {
    try {
        // Пробуем получить одно сообщение для проверки доступа к таблице
        ...
        if (error && error.code === '42P01') { // Код ошибки "отношение не существует"
            console.log('Таблица сообщений не существует, создаем...');
            
            // В случае ошибки таблицы, будем использовать локальное хранилище
            localStorage.setItem('chat_mode', 'local');
            
            // Предупреждаем пользователя
            addSystemMessageToUI('Подключение к общей базе данных не удалось. Чат работает в локальном режиме.');
        } else {
            localStorage.setItem('chat_mode', 'cloud');
            console.log('Таблица сообщений существует');
        }
    } catch (error) {
        console.error('Ошибка при проверке таблицы:', error);
        localStorage.setItem('chat_mode', 'local');
        addSystemMessageToUI('Ошибка подключения к базе данных. Чат работает в локальном режиме.');
    }
}
...
function initChat() {
    ...
    // Очищаем интервал синхронизации при закрытии страницы
    window.addEventListener('beforeunload', () => {
        if (messagesSyncInterval) {
            clearInterval(messagesSyncInterval);
        }
    });
}
...
function saveUsername() {
    ...
    // Проверим, существует ли уже такое имя пользователя
    checkUsernameExists(name).then(exists => {
        if (exists) {
...
async function checkUsernameExists(name) {
    // Проверка имен из локального хранилища
    const usedNames = JSON.parse(localStorage.getItem('chat_used_names') || '[]');
    
    if (usedNames.includes(name)) {
        return true;
    }
    
    // Если используем облачный режим, проверяем в Supabase
    if (localStorage.getItem('chat_mode') === 'cloud' && supabase) {
        try {
            const { data, error } = await supabase
                .from('messages')
                .select('author')
...
async function loadMessages() {
    // Если чат не инициализирован или нет имени, не загружаем
    if (!supabase) return;
    
    try {
        let messages = [];
        
        // Проверяем режим работы чата
        if (localStorage.getItem('chat_mode') === 'cloud') {
        ...
        // Если произошла ошибка загрузки из облака, переключаемся на локальный режим
        if (localStorage.getItem('chat_mode') === 'cloud') {
            localStorage.setItem('chat_mode', 'local');
            addSystemMessageToUI('Проблема с подключением к серверу чата. Переключились на локальный режим.');
        }
    }
}

// Обновление UI сообщениями
function updateMessagesUI(messages) {
    // Если это первая загрузка, очищаем контейнер
    if (chatMessages.querySelector('.system-message')?.textContent === 'Подключение к чату...') {
        chatMessages.innerHTML = '';
        
        if (messages.length === 0) {
            addSystemMessageToUI('Добро пожаловать в погодный чат!');
            addSystemMessageToUI('Здесь вы можете общаться с другими пользователями о погодных условиях.');
        }
    }
    
    // Проверяем, есть ли новые сообщения для добавления
    ...
    messages.forEach(message => {
        ...
        // Если сообщение новее последнего показанного, добавляем его
        if (messageTime > lastMessageTime) {
            if (message.system) {
                addSystemMessageToUI(message.text);
            } else {
                addMessageToUI(message, message.author === username);
            }
            
            // Обновляем время последнего сообщения
            if (messageTime > parseInt(localStorage.getItem('last_message_time') || '0')) {
                localStorage.setItem('last_message_time', messageTime.toString());
                hasNewMessages = true;
            }
        }
    });
    
    // Если есть новые сообщения и чат активен, прокручиваем вниз
    if (hasNewMessages && chatIsActive) {
        scrollToBottom();
    }
}
...
async function sendMessage() {
    ...
    try {
        ...
        if (localStorage.getItem('chat_mode') === 'cloud' && supabase) {
            ...
            if (localMessages.length > MESSAGES_LIMIT) {
                localMessages.splice(0, localMessages.length - MESSAGES_LIMIT);
            }
            ...
            if (!usedNames.includes(username)) {
                ...
                localStorage.setItem('chat_used_names', JSON.stringify(usedNames));
            }
        }
    } catch (error) {
        ...
        // Если произошла ошибка отправки в облако, переключаемся на локальный режим
        if (localStorage.getItem('chat_mode') === 'cloud') {
            localStorage.setItem('chat_mode', 'local');
            addSystemMessageToUI('Проблема с подключением к серверу чата. Переключились на локальный режим.');
            
            // Сохраняем сообщение локально
            sendMessageLocally(messageObj);
        }
    }
}

// Отправка сообщения локально
function sendMessageLocally(messageObj) {
    const localMessages = JSON.parse(localStorage.getItem('chat_messages') || '[]');
    localMessages.push(messageObj);
...
async function sendSystemMessage(text) {
    ...
    // Добавляем системное сообщение в UI
    addSystemMessageToUI(text);
...
// Добавление системного сообщения в UI
...
function limitMessagesInDOM() {
    while (chatMessages.children.length > MESSAGES_LIMIT) {
