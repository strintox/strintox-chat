...
function addEventListeners() {
    ...
    document.getElementById('start-chat-btn')?.addEventListener('click', handleStartChat);
    document.getElementById('message-form')?.addEventListener('submit', handleSendMessage);
    document.getElementById('search-input')?.addEventListener('input', handleSearchChats);
    document.getElementById('voice-msg-btn')?.addEventListener('click', handleVoiceMessage);
    document.getElementById('cancel-recording-btn')?.addEventListener('click', cancelVoiceRecording);
    document.getElementById('stop-recording-btn')?.addEventListener('click', stopVoiceRecording);
    
    // Search options in new chat modal
    document.getElementById('search-by-username')?.addEventListener('click', switchToUsernameSearch);
    document.getElementById('search-by-email')?.addEventListener('click', switchToEmailSearch);
    
    // Voice and video calls
    document.getElementById('voice-call-btn')?.addEventListener('click', initiateCall);
    document.getElementById('end-call-btn')?.addEventListener('click', endCall);
    document.getElementById('answer-call-btn')?.addEventListener('click', answerCall);
    document.getElementById('mute-call-btn')?.addEventListener('click', toggleMute);
    
    // Mobile-specific events
    document.getElementById('menu-toggle')?.addEventListener('click', toggleSidebar);
    document.getElementById('show-chats-btn')?.addEventListener('click', showSidebar);
    
    // Theme toggle
    document.getElementById('theme-toggle')?.addEventListener('click', toggleTheme);
    
    // Add input focus events for better mobile experience
    if (messageInput) {
...
function initMobileOptimizations() {
    if (isMobile) {
        // Ensure all interactive elements have proper touch events
        document.querySelectorAll('button, .chat-item, .btn, .btn-icon').forEach(element => {
        ...
        window.addEventListener('resize', () => {
            // Update vh value when keyboard appears/disappears
            const vh = window.innerHeight * 0.01;
            document.documentElement.style.setProperty('--vh', `${vh}px`);
        });
    }
...
function toggleTheme() {
    ...
    if (metaThemeColor) {
        metaThemeColor.setAttribute('content', currentTheme === 'dark' ? '#1D2432' : '#FFFFFF');
    }
}
...
// Load messages for the active chat - улучшенная версия
function loadMessages(chatId) {
    ...
    messagesListener = messagesQuery.onSnapshot((snapshot) => {
        ...
        // Прокручиваем только если пользователь был внизу контейнера или это новые сообщения
        if (shouldScroll) {
    ...
    // Объединяем слушатели для удобства очистки
...
function displayMessage(messageData) {
    ...
    // Проверяем, нужно ли прокручивать к новому сообщению
    ...
    if (messageData.senderId === 'system' || messageData.type === 'system') {
        ...
        if (playButton) {
            ...
            playButton.addEventListener('touchstart', function(e) {
                e.preventDefault(); // Предотвращаем задержку на мобильных
                handlePlay();
            }, { passive: false });
        }
    } else {
        ...
        // Гарантируем, что текст сообщения не сломает верстку
    ...
    // Вставляем сообщение и прокручиваем, если нужно
    messagesContainer.appendChild(messageElement);
    
    if (shouldScroll) {
        scrollToBottom(!isMobile); // На мобильных используем мгновенную прокрутку
    }
}
...
// Улучшенная функция прокрутки сообщений до конца чата
function scrollToBottom(smooth = true) {
    if (!messagesContainer) return;
    
    // Задержка для уверенности, что все сообщения отрендерены
    setTimeout(() => {
        const maxScroll = messagesContainer.scrollHeight - messagesContainer.clientHeight;
        
        if (maxScroll > 0) {
            // На мобильных устройствах прокрутка должна быть мгновенной
            messagesContainer.scrollTo({
                top: maxScroll,
                behavior: isMobile ? 'auto' : (smooth ? 'smooth' : 'auto')
            });
...
async function startVoiceRecording() {
    try {
        ...
        document.getElementById('voice-recording-container').classList.remove('hidden');
        ...
        const timerInterval = setInterval(updateRecordingTime, 1000);
        ...
        // Store everything in a global object for access in other functions
        ...
        const recordingWaves = document.querySelectorAll('.voice-wave span');
        recordingWaves.forEach((wave, index) => {
            wave.style.animation = `wave 1s infinite ease-in-out ${index * 0.2}s`;
        });
        ...
        document.getElementById('voice-recording-container').classList.add('hidden');
...
async function sendVoiceMessage(audioUrl, duration) {
    ...
    try {
        ...
        const loadingMessage = {
            content: 'Отправка голосового сообщения...',
            senderId: 'system',
        ...
        await db.collection('chats').doc(activeChatId).collection('messages').add(messageData);
        
        // Обновляем последнее сообщение в чате
        await db.collection('chats').doc(activeChatId).update({
            lastMessage: `Голосовое сообщение (${formatDuration(duration)})`,
            lastMessageTimestamp: firebase.firestore.Timestamp.now(),
            lastMessageSenderId: currentUser.uid
        });
        
        // Удаляем индикатор загрузки (ищем последнее системное сообщение)
        const systemMessages = messagesContainer.querySelectorAll('.message-system');
        if (systemMessages.length > 0) {
            systemMessages[systemMessages.length - 1].remove();
        }
        
        // Вибрация на мобильных для подтверждения отправки
        ...
        // Показываем ошибку вместо индикатора загрузки
        const systemMessages = messagesContainer.querySelectorAll('.message-system');
        if (systemMessages.length > 0) {
            const lastMessage = systemMessages[systemMessages.length - 1];
            lastMessage.classList.add('error');
            lastMessage.querySelector('.message-text').textContent = 'Ошибка при отправке сообщения';
        }
...
// Закрепляем сообщение о загрузке на активный диалог
document.getElementById('show-chats-btn').addEventListener('click', function() {
    if (sidebarElement) {
        sidebarElement.classList.add('show');
    }
});

// Фикс для обработки прокрутки при открытии клавиатуры на мобильных устройствах
window.addEventListener('resize', function() {
    if (messagesContainer && window.innerWidth <= 768) {
        // Используем небольшую задержку, чтобы дать клавиатуре время развернуться
        setTimeout(() => {
            scrollToBottom(false);
        }, 300);
    }
...
// Проверяем, является ли устройство мобильным при загрузке страницы
document.addEventListener('DOMContentLoaded', function() {
    if (isMobile) {
        document.body.classList.add('mobile-device');
        
        // Предотвращаем двойной тап на сенсорных устройствах
        document.addEventListener('touchend', function(e) {
            e.preventDefault();
            // Проверяем, что клик был на кнопке
            if (e.target.closest('button') || e.target.tagName === 'BUTTON') {
                e.target.closest('button') ? e.target.closest('button').click() : e.target.click();
            }
        }, { passive: false });
    }
    
    // Fix для iOS scrolling
    if (isIOS) {
...
// Add this to ensure audio playback works on mobile
document.addEventListener('DOMContentLoaded', function() {
    // Create a silent audio context on first user interaction to unlock audio
    let audioContext;
